    ...
    <div className="pw-page" style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem', display: 'flex', gap: '2rem' }}>
          ...
          {CATEGORIES.map((cat) => { 
            ...
            return (
              <button key={cat.name} className={`pw-sidebar-item ${selected === slug ? 'is-active' : ''}`} onClick={() => setSelected(slug)}>
                ...
                <span className="count">{prompts.filter((p) => p.category === slug).length}</span>
              </button>
            ); 
          })}
        </div>
      ...
      <main style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ position: 'relative', marginBottom: '2rem' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9a9aab" strokeWidth="2" style={{ position: 'absolute', left: 15, top: '50%', transform: 'translateY(-50%)' }}>
      ...
      {/* RIGHT RAIL */}
      <aside className="pw-rightrail" style={{ width: 300, flexShrink: 0, position: 'sticky', top: 100, height: 'fit-content', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {/* Right Ad: Native Banner */}
        <AdSlot code={ADS.right} label="Sponsored" />
        {/* Smartlink Ad */}
        <AdSlot code={ADS.smart} label="Sponsored partner" className="pw-smart" />
        
        <div className="pw-promo" style={{ background: 'var(--panel)', padding: '1.5rem', borderRadius: 16, border: '1px solid var(--line)' }}>
          <span className="pw-promo__kicker" style={{ color: 'var(--muted)', fontSize: 12, fontWeight: 600, letterSpacing: '0.05em' }}>PromptWorld+</span>
          <h4 className="pw-promo__title" style={{ color: '#fff', fontSize: 18, fontWeight: 700, margin: '8px 0' }}>Unlock the full archive</h4>
          <p className="pw-promo__text" style={{ color: 'var(--muted)', fontSize: 14, lineHeight: 1.5, margin: '0 0 1rem' }}>Premium prompts, early drops, and the complete recipe library — ad‑light.</p>
          <Link href="/subscribe" className="pw-btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '10px', display: 'flex' }}>Go Premium</Link>
        </div>
      </aside>

      <MobileCategoryBar selected={selected} onSelect={setSelected} />
      
      {/* Site-wide Ads (Popunder & Social Bar) */}
