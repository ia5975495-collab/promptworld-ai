...
export async function POST(req: Request) {
  ...
  if (!body) { console.error('[publish] empty body'); return NextResponse.json({ error: 'bad request' }, { status: 400 }); }
  ...
  if (typeof password !== 'string' || !safeEq(password, admin)) {
    console.error('[publish] unauthorized — password blank or mismatched');
    return NextResponse.json({ error: 'unauthorized' }, { status: 401 });
  }
  if (!url || !role) {
    console.error('[publish] cloud env missing', { hasUrl: !!url, hasRole: !!role });
    return NextResponse.json({ error: 'cloud not configured' }, { status: 503 });
  }

  const supa = createClient(url, role);
  let image_url = prompt?.image_url || '';

  if (typeof imageDataUrl === 'string' && imageDataUrl.startsWith('data:image')) {
    try {
      const base64 = imageDataUrl.split(',')[1] || '';
      const buf = Buffer.from(base64, 'base64');
      const path = `${prompt.id || 'u' + Date.now()}.jpg`;
      const { error } = await supa.storage.from(BUCKET).upload(path, buf, { contentType: 'image/jpeg', upsert: true });
      if (error) { console.error('[publish] storage upload error:', error.message); return NextResponse.json({ error: 'upload failed: ' + error.message }, { status: 500 }); }
      image_url = supa.storage.from(BUCKET).getPublicUrl(path).data.publicUrl;
    } catch (e: any) { console.error('[publish] upload exception:', e?.message || e); return NextResponse.json({ error: 'upload error: ' + (e?.message || 'unknown') }, { status: 500 }); }
  }
  ...
  const row = {
    ...
    creator_name: prompt.creator_name || 'Admin',
    image_url,
    likes_count: 0,
    downloads_count: 0,
    views_count: 0,
  ...
  console.log('[publish] ✅ success id =', data?.id);
